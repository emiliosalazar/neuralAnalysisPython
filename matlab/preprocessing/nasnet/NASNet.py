...
def trainnet(netname,ntimepts,traindir):
    ...
    #  TRAIN MODEL
    trainingFiles = []
    for file in os.listdir(traindir)[::-1]:  # pull all training files from directory
    ...
    for j in range(nsets):  # loop over the different training files
        ...
        varName = 'outputMatLbFirstCol'
        # varName = 'waveData'
        print('Loading waveform set {}\n'.format(j))
        varNamesInMat = list(f_waves.keys())
        if len(varNamesInMat)>1:
            raise Exception("Too many variables in file")
        else:
            varName = varNamesInMat[0]
        waveforms = np.transpose(f_waves[varName][()])
