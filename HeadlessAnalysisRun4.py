...
#keyStateName = 'stimulus'
#binnedSpikesShortStartOffshift, bssiKeys = genBSLAroundState(dsi,
#                                    keyStateName,
#                                    trialType = trialType,
#                                    lenSmallestTrl=lenSmallestTrl, 
#                                    binSizeMs = binSizeMs, 
#                                    furthestTimeBeforeState=-offshift, # note that this starts it *forwards* from the delay
#                                    furthestTimeAfterState=-offshift,
...
#
keyStateName = 'delay'
# I'm seeing some interesting Fano factors when computed across the entire
# dataset (relating to a) not all the trials being the same length, b) as a
# result not all the trials being fractionally split up among epochs the same
# way... so in any case, instead of manipulating the function to deal with
# these special cases, I'm going to grab bin spike sets with a super-high fano
# that will hopefully not get rid of them, and then do the special fano factor
# thresh later on; also there's a possibility values >100 are found, but I feel
# like channels with that fano would be... extremely weird in any case
fanoFactorThresh = 100
binnedSpikesAll, bssiAllKeys = genBSLAroundState(dsi['dataset_id=0'],
                                    keyStateName,
                                    trialType = trialType,
                                    lenSmallestTrl=lenSmallestTrl, 
                                    binSizeMs = binSizeMs, 
...
minNumNeurons = 60
bssExp = bsi[bssiAllKeys][fsp['ch_num>%d' % minNumNeurons]]
...
bnSp = binnedSpikesList[0]
plotTypes = {}
plotTypes.update({'raster' : dict(
                        plotSegments=True,
                        ptsAroundAlBin = binSizeMs*(np.min([furthestBack,furthestForward])/binSizeMs).astype('int')
                        )
            })
#plotTypes.update({'psth' : dict(
#                        plotSegments=True,
#                        ptsAroundAlBin = (np.min([furthestBack,furthestForward])/binSizeMs).astype('int')
#                        )
#            })
bssExp.generateDescriptivePlots(plotTypes = plotTypes)
saveFiguresToPdf(pdfname="chAllRastDelay")
plt.close('all')
...
#saveFiguresToPdf(pdfname="GPFAManyAreas")
...
saveFiguresToPdf(pdfname="genericMetricsStimPeriod")
plt.close('all')
...
if plotResid:
    ...
    saveFiguresToPdf(pdfname="residualsOverSubsetsStimPeriod")
    plt.close('all')
...
#                breakpoint()
                [plt.scatter(m1, m2, label=desc, color=colorsUse[colN,:], marker=(mcN+2, 0, ang*rotDegBtShapes)) for m1, m2, desc, colN, mcN, ang in zip(metricVal, metric2Val, descriptions, colNum, mcNum, angle)]
...
pltAllVsAll(descriptions, brainAreas, tasks, dsNames, metricDict)
#
#plt.figure()
...
saveFiguresToPdf(pdfname='scatterMetricsOverSubsetsStimPeriod')
